...
OUT='build_current_master'
os.makedirs(OUT, exist_ok=True)

# PX-1 Rev.B CURRENT MASTER
...
# rear motor housing reconciled to existing supported two-motor input screen
REAR_X0=242.0
REAR_X1=384.0
REAR_Y_HALF=46.0
REAR_Z0=19.0
...
# corrected manual lift / camera geometry
BODY_PIVOT_X=200.0
PIVOT_Z_LOW=92.0
PIVOT_Z_HIGH=109.0
PIVOT_AVG=100.5
...
# compact Proteus-like service pressure valve envelope
VALVE_X=273.0
CAP_OD=12.0
CAP_H=1.5
CARTRIDGE_OD=10.0
...
# exact six-function local camera harness
HARNESS_OD=6.5
HARNESS_Y=25.5
GUARD_OD=10.0
ARM_RUN=(20.0,72.0)

# dry six-way connector and wet camera connector envelopes
MICROFIT_CX=247.0
MICROFIT_CZ=101.5
MICROFIT_L=14.0
MICROFIT_W=12.0
...
# rear tether tail functional envelope based on ASS-002-090 / ASS-002-364
TAIL_X0=384.0
TAIL_X1=424.0
TAIL_OD=20.0
...
body=cut(body,prism_x(221,pod_inner_pts,83))
body=cut(body,box0(224,-33,84,76,66,10))
body=cut(body,boxc(COVER_CX,0,108.0,OPEN_L,OPEN_W,8.0))
...
for side in (-1,1):
    for x in WHEEL_X:
        # visual proxy only: exact QRW90SR/150 outer profile is a release gate
    ...
    y0=side*(HALF_OUT-1.0); direction=1 if side>0 else -1
    for x in WHEEL_X:
        axles.append((f'axle_{side}_{int(x)}',wp(cq.Solid.makeCylinder(6.0,16.0,cq.Vector(x,y0,WHEEL_Z),cq.Vector(0,direction,0)))))
...
for side in (-1,1):
    ...
    motor_parts += [(f'motor_{side}',motor),(f'motor_shaft_{side}',shaft),(f'coupling_{side}',coupling),(f'pinion_bearing_{side}',bearing),(f'Z16_pinion_envelope_{side}',pinion)]
...
# pressure service / camera harness
...
# tail / main tether functional envelope
...
collision={
    'top_vs_wheels':max(inter(p,w) for _,p in fixed_top for _,w in wheels),
    'top_vs_gears':max(inter(p,g) for _,p in fixed_top for _,g in gears),
    'top_vs_arms':max(inter(p,a) for _,p in fixed_top for _,a in arms),
    'top_vs_camera':max(inter(p,camera_outer) for _,p in fixed_top),
    'guard_vs_gears':max(inter(guard,g) for _,g in gears),
    'guard_vs_camera':inter(guard,camera_outer),
    'motor_vs_camera':max(inter(p,camera_outer) for n,p in motor_parts if n.startswith('motor_')),
}
pipe_clear={
...
wheel_source_compatibility=True   # MiniCam QRW90SR/150 -> recommended 150 mm pipe
wheel_geometry_release=False      # exact FSS-002-065 / QRW90SR/150 outer profile not recovered
master_geometry_ok=architecture_ok and collision_ok and nonwheel_pipe_ok and wheel_source_compatibility
status=('PASS_INTEGRATED_MASTER / WHEEL_GEOMETRY_DN150_HOLD / PRESSURE_TEST_HOLD / PROCUREMENT_HOLD' if master_geometry_ok else 'FAIL_INTEGRATED_MASTER')
...
result={
    ...
    'source_architecture':{
        ...
        'tail':'ASS-002-090 / ASS-002-364 functional connector and strain-relief architecture'
    },
    'counts':counts,
    'hard_dimensions_mm':{
    ...
    'release_holds':[
        'exact matched Z16/Z40 geometry and mounting distance remains sample/drawing gate',
        'rear tail exact dimensions remain to freeze; current model follows recovered source functional stack',
        'exact QRW90SR/150 wheel profile remains required for final geometric DN150 release',
        ...
        'existing full TILT sweep remains controlling for camera clearance; this master shows LOW static envelope',
        'no machining release until physical pressure and DN150 jig tests pass'
    ]
}
with open(os.path.join(OUT,'PX1_CURRENT_MASTER_VALIDATION.json'),'w',encoding='utf-8') as f:
