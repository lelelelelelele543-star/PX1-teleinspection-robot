...
# PX-1 Rev.GV — sealed open wet-deck + revised internal packaging
# Prototype engineering only; NOT machining release.

OUT=os.path.abspath('build_revgv')
os.makedirs(OUT,exist_ok=True)

L=307.0; W=92.0; Z0=8.0; ZTOP=90.0
PIPE_R=75.0; PIPE_Z=52.0480547
HALF_OUT=46.0; HALF_IN=34.0
FLOOR=6.0; END=8.0
WHEEL_Z=45.0
BAY_X0=10.5; BAY_L=286.0; BAY_Z0=6.0; BAY_H=80.0; BAY_DEPTH=8.0; BAY_INNER_Y=38.0
DECK_HALF_W=38.0
ROOF_T=5.0
# Monotonic wet-deck roof profile: low under folded camera, rising under lower lift arm,
# then closing flush with the normal body roof behind the pivot.
...
# One continuous dry P0 cavity; its upper surface follows the wet-deck structural roof.
...
body=cut(body,inner)
# Open wet deck. It is open upward and at X=0; no front wall/cup exists.
...
# Side dry gear bays remain untouched above Y=+/-38.
...
# Existing X200 interfaces remain below the rising pressure roof.
for sgn in (1,-1):
...
# Rear motor pressure extension, retained from the GL/GP packaging line.
rear_outer=box(299,-40,23,41,80,44)
body=fuse(body,rear_outer)
rear_cav=box(299,-34,27,33,68,36)
body=cut(body,rear_cav)

# New rear-top electronics saddle. It keeps the full NUCLEO out of the wet-deck volume
# without lengthening the wheelbase/body width. Low-profile headers/harness are required.
epod_outer=box(218,-39,90,89,78,20)
body=fuse(body,epod_outer)
epod_cav=box(221,-35.5,92,83,71,14)
body=cut(body,epod_cav)
throat=box(224,-33,84,76,66,10)
body=cut(body,throat)

# Combined dry-volume reference used for packaging checks.
cavity=wp(inner.val().fuse(rear_cav.val()).fuse(epod_cav.val()).fuse(throat.val()))

# ---- active LOW camera/lift geometry from Rev.FN ----
...
parts={
 'AVD_video_TX':bcenter(42.5,0,24,55,20,12),
 'DC48_24_halfbrick':bcenter(115,0,24,70,65,18),
 'TB6612_camera_axes':bcenter(175,0,28,50,25,19),
 'INPUT_PROTECTION_reserve':bcenter(175,0,49,22,60,22),
 # One compact dual traction driver candidate. Exact article/stock remains HOLD.
 'DUAL_MC33926_candidate':bcenter(219,-10,50,28,46,12),
 # Full NUCLEO board is retained, moved to the dry rear-top saddle.
 # 12 mm is a low-profile installation target after removing tall plug headers.
 'NUCLEO_F446RE_lowprofile':bcenter(262.25,0,99,82.5,70,12),
 'MOTOR_L_32x92':cylx_center(283,16.5,45,16,92),
 'MOTOR_R_32x92':cylx_center(283,-16.5,45,16,92),
 # PTW1CM-class sensor, mounted vertically in the rear throat.
...
checks={}
checks['body_valid']=body.val().isValid()
checks['body_outside_ideal_DN150_mm3']=round(body.val().cut(pipe.val()).Volume(),6)
checks['camera_body_intersection_mm3']=round(inter(camera,body),6)
checks['arm_body_intersection_mm3']=[round(inter(a,body),6) for a in arms]
checks['component_outside_dry_volume_mm3']={k:round(outside(v,cavity),6) for k,v in parts.items()}
...
checks['wet_deck_half_width_mm']=DECK_HALF_W
checks['camera_side_clearance_each_mm']=round(DECK_HALF_W-CAM_OD/2,3)
checks['camera_under_gap_front_center_rear_mm']=[
    round(CAM_Z-CAM_OD/2-roof_top(lens_x),3),
    round(CAM_Z-CAM_OD/2-roof_top(camx),3),
    round(CAM_Z-CAM_OD/2-roof_top(camx+CAM_LEN/2),3)]
checks['forward_opening_horizontal_cone_deg_screen']=round(2*math.degrees(math.atan2(DECK_HALF_W,lens_x)),3)
# Lower arm to monotonic roof clearance.
mingap=1e9
for i in range(201):
...
checks['roof_profile_xz_mm']=ROOF_PTS
checks['roof_thickness_mm']=ROOF_T
checks['top_saddle_DN150_corner_margin_mm']=round(PIPE_R-math.hypot(39,110-PIPE_Z),3)
checks['drainage']='Monotonic deck roof to open nose; no rear vertical cup. Smooth radii/physical sludge test remain release gates.'
checks['status']='PASS' if (checks['body_valid'] and checks['body_outside_ideal_DN150_mm3']<1e-5 and checks['camera_body_intersection_mm3']<1e-5 and all(v<1e-5 for v in checks['arm_body_intersection_mm3']) and all(v<1e-5 for v in checks['component_outside_dry_volume_mm3'].values()) and not checks['component_intersections_mm3']) else 'FAIL'
checks['release_holds']=['actual NUCLEO installed height after header strategy','exact compact dual traction driver article/stock','fillets/radii on wet-deck ramps','full pressure FEA','physical water/sludge drainage test','actual camera lens FOV check']

cq.exporters.export(body,os.path.join(OUT,'PX1_PressureBody_WetDeck_RevGV.step'))
assy=cq.Assembly(name='PX1_RevGV_Packaging')
assy.add(body,name='PressureBody_WetDeck')
...
assy.save(os.path.join(OUT,'PX1_RevGV_Packaging.step'))
with open(os.path.join(OUT,'REV_GV_VALIDATION.json'),'w') as f: json.dump(checks,f,indent=2)
