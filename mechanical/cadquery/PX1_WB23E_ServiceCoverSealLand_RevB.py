...
# PX-1 Rev.B WB23E — seal-land correction and full integration of the compact pressure/service cover.
# Validation only. Not a machining release.
OUT=os.path.abspath('build_wb23e'); os.makedirs(OUT, exist_ok=True)

# ---- hard architecture / WB22A ----
...
# camera conservative outer envelope from WB22A
HEAD_R=26.0; HEAD_L=78.0; BOSS_OUTER_Y=34.0; TILT_POD_OD=18.0; TILT_WHEEL_Y=-16.75; WORM_CD=14.5; TILT_POD_X=56.0

# ---- WB23E corrected service-cover package ----
# WB23C/D used too little material between the M4 clearance holes and a realistic face-seal groove.
# WB23E enlarges the plate/land and reduces the opening while staying inside the pressure pod.
...
HARNESS_OD=5.0; HARNESS_Y=25.5; GUARD_OD=7.0; ARM_RUN=(20.0,72.0)
...
body_with_service=cut(body,service_open)
assert body_with_service.val().isValid()
...
open_x0=COVER_CX-OPEN_L/2; open_x1=COVER_CX+OPEN_L/2
open_y0=-OPEN_W/2; open_y1=OPEN_W/2
...
groove_outer_half_x=OPEN_L/2+GROOVE_OFFSET_FROM_OPEN+GROOVE_W/2
groove_outer_half_y=OPEN_W/2+GROOVE_OFFSET_FROM_OPEN+GROOVE_W/2
...
DRY_CONNECTOR_L=31.62; DRY_CONNECTOR_W_SCREEN=10.5
dry_connector_fits_opening=(DRY_CONNECTOR_L<=OPEN_L and DRY_CONNECTOR_W_SCREEN<=OPEN_W)

worst={
    'fixed_vs_Z50_mm3':0.0,
    'fixed_vs_wheels_mm3':0.0,
    'fixed_vs_lift_arms_mm3':0.0,
    'fixed_vs_camera_outer_mm3':0.0,
    'guard_vs_fixed_mm3':0.0,
    'guard_vs_Z50_mm3':0.0,
}
states={}
for name,zcam in CAM_ZS.items():
    ...
    cam=camera_outer(cam_x_for(zcam),zcam)
    th=theta_for(zcam)
    def arm_point(s): return (BODY_PIVOT_X-s*math.cos(th),HARNESS_Y,PIVOT_Z_LOW+s*math.sin(th))
    a=arm_point(ARM_RUN[0]); b=arm_point(ARM_RUN[1])
    guard=cyl_between(a,b,GUARD_OD/2)
    fixed_arm=max(inter(fp,a_) for _,fp in fixed for a_ in arms)
    fixed_cam=max(inter(fp,cam) for _,fp in fixed)
    guard_fixed=max(inter(guard,fp) for _,fp in fixed)
    guard_gear=max(inter(guard,g) for g in gears)
    worst['fixed_vs_lift_arms_mm3']=max(worst['fixed_vs_lift_arms_mm3'],fixed_arm)
    worst['fixed_vs_camera_outer_mm3']=max(worst['fixed_vs_camera_outer_mm3'],fixed_cam)
    worst['guard_vs_fixed_mm3']=max(worst['guard_vs_fixed_mm3'],guard_fixed)
    worst['guard_vs_Z50_mm3']=max(worst['guard_vs_Z50_mm3'],guard_gear)
    states[name]={
        'camera_axis_xz_mm':[cam_x_for(zcam),zcam],
        'guard_dn150_clearance_mm':radial_clear(guard),
        'fixed_vs_arms_mm3':fixed_arm,
        'fixed_vs_camera_mm3':fixed_cam,
        'guard_vs_fixed_mm3':guard_fixed,
        'guard_vs_Z50_mm3':guard_gear,
    }

worst['fixed_vs_Z50_mm3']=max(inter(fp,g) for _,fp in fixed for g in gears)
worst['fixed_vs_wheels_mm3']=max(inter(fp,w) for _,fp in fixed for w in wheels)
fixed_dn150={n:radial_clear(p) for n,p in fixed}

A=OPEN_L*OPEN_W
pressure_loads={'0.25bar_N':A*0.025,'1bar_N':A*0.1,'1bar_each_M4_N':A*0.1/2}
collision_ok=all(v<1e-4 for v in worst.values())
checks_ok=(
    body_with_service.val().isValid() and opening_inside_pod and cover_supported_by_pod and gland_crosses_front_wall and
    screw_to_groove_web>=3.0 and head_to_cover_edge>=3.0 and groove_to_cover_edge_y>=4.0 and dry_connector_fits_opening and
    collision_ok and min(fixed_dn150.values())>=3.0 and states['LOW']['guard_dn150_clearance_mm']>=5.0
)
status='PASS_SEAL_LAND_AND_FULL_INTEGRATION_SCREEN / SEAL_TEST_HOLD / FLEX_TEST_HOLD / PROCUREMENT_HOLD' if checks_ok else 'FAIL_SEAL_LAND_OR_INTEGRATION_SCREEN'

result={
    'status':status,
    'architecture':{'wheels':len(wheels),'z50_gears':len(gears),'wheel_x_mm':list(WHEEL_X),'body_pivot_x_mm':BODY_PIVOT_X},
    'service_cover':{
        'outer_LxW_mm':[COVER_L,COVER_W],
        'x_range_mm':[cover_x0,cover_x1],
        'supported_by_pressure_pod':cover_supported_by_pod,
        'opening_LxW_mm':[OPEN_L,OPEN_W],
        'opening_x_range_mm':[open_x0,open_x1],
        'opening_y_range_mm':[open_y0,open_y1],
        'opening_inside_pressure_pod':opening_inside_pod,
        'body_valid_after_cut':body_with_service.val().isValid(),
        'screw_x_mm':list(SCREW_X),
        'provisional_groove_offset_from_opening_mm':GROOVE_OFFSET_FROM_OPEN,
        'provisional_groove_width_mm':GROOVE_W,
        'minimum_screw_hole_to_groove_web_mm':screw_to_groove_web,
        'minimum_screw_head_to_cover_edge_mm':head_to_cover_edge,
        'minimum_groove_to_cover_y_edge_mm':groove_to_cover_edge_y,
        'dry_connector_screen_LxW_mm':[DRY_CONNECTOR_L,DRY_CONNECTOR_W_SCREEN],
        'dry_connector_fits_opening':dry_connector_fits_opening
    },
    'gland':{'crosses_pressure_pod_front_wall':gland_crosses_front_wall,'axis_yz_mm':[GLAND_AXIS_Y,GLAND_AXIS_Z]},
    'fixed_dn150_clearance_mm':fixed_dn150,
    'states':states,
    'worst_unintended_collision_mm3':worst,
    'pressure_load_screen':pressure_loads,
    'release_note':'PASS is packaging/integration only. Final service-cover seal, bosses, fasteners, gland, fill valve and local harness require real-part qualification.'
}
with open(os.path.join(OUT,'REV_B_WB23E_VALIDATION.json'),'w') as f: json.dump(result,f,indent=2)
