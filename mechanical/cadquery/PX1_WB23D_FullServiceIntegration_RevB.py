...
# PX-1 Rev.B WB23D — integrate WB23C service cover into exact WB22A body/lift screen.
# Validation only. Not a machining release.
OUT=os.path.abspath('build_wb23d'); os.makedirs(OUT, exist_ok=True)
...
# ---- WB23C package ----
COVER_CX=257.0; COVER_CY=0.0; COVER_Z0=110.0; COVER_L=78.0; COVER_W=42.0; COVER_T=6.0
OPEN_L=58.0; OPEN_W=24.0
SCREW_X=(224.0,290.0); SCREW_Y=0.0; SCREW_HEAD_D=8.0; SCREW_HEAD_H=3.0
PRESSURE_PORT_CX=268.0; PRESSURE_PORT_CY=0.0; PRESSURE_PORT_OD=14.0; PRESSURE_PORT_H=6.0
...
# service opening through top pressure roof, then verify body remains valid
...
checks_ok=(
    body_with_service.val().isValid() and opening_inside_pod and gland_crosses_front_wall and
    collision_ok and min(fixed_dn150.values())>=3.0 and states['LOW']['guard_dn150_clearance_mm']>=5.0
)
status='PASS_FULL_INTEGRATION_SCREEN / SEAL_TEST_HOLD / FLEX_TEST_HOLD / PROCUREMENT_HOLD' if checks_ok else 'FAIL_FULL_INTEGRATION_SCREEN'

result={
    'status':status,
    'architecture':{'wheels':len(wheels),'z50_gears':len(gears),'wheel_x_mm':list(WHEEL_X),'body_pivot_x_mm':BODY_PIVOT_X},
    'service_opening':{'inside_pressure_pod':opening_inside_pod,'x_range_mm':[open_x0,open_x1],'y_range_mm':[open_y0,open_y1],'body_valid_after_cut':body_with_service.val().isValid()},
...
with open(os.path.join(OUT,'REV_B_WB23D_VALIDATION.json'),'w') as f: json.dump(result,f,indent=2)
